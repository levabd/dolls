...
var modeObjectE : GameObject;
var modeObjectF : GameObject;
var targetObjectE : Transform;
var targetObjectF1 : Transform;
var targetObjectF2 : Transform;
...
private var targetRenderer1 : Renderer;
private var targetRenderer2 : Renderer;
...
private var colorHighlight = Color(0.99,0.75,0.074,1.0);

private var autoDilateObject : EyeAdv_AutoDilation;
private var autoDilateObject1 : EyeAdv_AutoDilation;
private var autoDilateObject2 : EyeAdv_AutoDilation;
...
function Start () {
	...
	if (targetObjectE != null){
		targetRenderer = targetObjectE.transform.GetComponent(Renderer) as Renderer;
		autoDilateObject = targetObjectE.gameObject.GetComponent(EyeAdv_AutoDilation) as EyeAdv_AutoDilation;
	}
	//if (targetObjectF1 != null){
	//	targetRenderer1 = targetObjectF1.transform.GetComponent(Renderer) as Renderer;
	//	autoDilateObject1 = targetObjectF1.gameObject.GetComponent(EyeAdv_AutoDilation) as EyeAdv_AutoDilation;
	//}
	//if (targetObjectF2 != null){
	//	targetRenderer2 = targetObjectF2.transform.GetComponent(Renderer) as Renderer;
	//	autoDilateObject2 = targetObjectF2.gameObject.GetComponent(EyeAdv_AutoDilation) as EyeAdv_AutoDilation;
	//}

	if (targetObjectE != null) targetRenderer = targetObjectE.transform.GetComponent(Renderer) as Renderer;
	if (targetObjectF1 != null) targetRenderer1 = targetObjectF1.transform.GetComponent(Renderer) as Renderer;
	if (targetObjectF2 != null) targetRenderer2 = targetObjectF2.transform.GetComponent(Renderer) as Renderer;
...
function OnGUI(){
	...
	GUI.Label (Rect (160, 128, 280, 20), "CHARACTER VIEW (not included)");
	...
	if (Event.current.type == EventType.MouseUp && Rect(35,128,100,20).Contains(Event.current.mousePosition)) sceneMode = "eye";
	if (Event.current.type == EventType.MouseUp && Rect(160,128,100,20).Contains(Event.current.mousePosition)) sceneMode = "figure";
	...
	if (Rect(60,215,40,20).Contains(Event.current.mousePosition)) GUI.color = colorHighlight;
	GUI.Label (Rect(60, 215, 40, 20), "LOD 0");
	if (Event.current.type == EventType.MouseUp && Rect(60,215,100,20).Contains(Event.current.mousePosition)) lodLevel = 0.0;
	GUI.color = colorGold;
	if (lodLevel<0.32 || lodLevel>0.70) GUI.color = colorGrey;
	if (Rect(165,215,50,20).Contains(Event.current.mousePosition)) GUI.color = colorHighlight;
	GUI.Label (Rect(165, 215, 50, 20), "LOD 1");
	if (Event.current.type == EventType.MouseUp && Rect(165,215,100,20).Contains(Event.current.mousePosition)) lodLevel = 0.5;
	GUI.color = colorGold;
	if (lodLevel<0.70) GUI.color = colorGrey;
	if (Rect(270,215,50,20).Contains(Event.current.mousePosition)) GUI.color = colorHighlight;
	GUI.Label (Rect(270, 215, 50, 20), "LOD 2");
	if (Event.current.type == EventType.MouseUp && Rect(270,215,100,20).Contains(Event.current.mousePosition)) lodLevel = 1.0;
	...
	if (Rect(240,278,40,20).Contains(Event.current.mousePosition)) GUI.color = colorHighlight;
	GUI.Label (Rect(240, 278, 40, 20), "auto");
	GUI.color = colorGold;
	if (autoDilate) GUI.color = colorGrey;
	if (Rect(280,278,40,20).Contains(Event.current.mousePosition)) GUI.color = colorHighlight;
	...
	for (var t : int = 0; t < irisTextures.length; t++){
		if (texDiv1 != null) GUI.Label(Rect (36+(t*22),416, texDiv1.width,texDiv1.height), texDiv1);
	}
