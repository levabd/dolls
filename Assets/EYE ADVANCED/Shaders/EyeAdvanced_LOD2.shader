Shader "EyeAdvanced/EyeAdvanced_LOD2" {
Properties {
	...
	_specsize("Specular", Range(0.0,1.0)) = 0.45
...
void surf (Input IN, inout SurfaceOutputStandardSpecular o) {
	...
	//o.Normal = eBump;//lerp(eBump,cBump,irismasktex);
	...
	half h3 = max(0,dot(o.Normal,IN.lightDir));
	...
	//##  UNITY 5 Features  ##
	//-------------------------
	o.Smoothness = lerp(_smoothness,0.95,irismasktex);
	o.Specular = lerp(0.05, saturate(lerp(_reflectTerm*1.1,_reflectTerm*1.2,irismasktex)), tex2D(_IrisTex,IN.uv_MainTex).r);


	//-------------------------------
	//##  LIGHT TERM CALCULATION  ##
	//-------------------------------
	half3 useNormal = o.Normal;

    half NdotV = max(0,dot(useNormal,IN.viewDir));
    half NdotV2 = max(0,dot(useNormal,IN.viewDir));
    half cNdotV = saturate(lerp(-0.1,1.5,max(0,dot(useNormal,IN.viewDir))));
	...
	half h2 = max(0,dot(useNormal,normalize(normalize(half3(IN.lightDir.x,IN.lightDir.y,-IN.lightDir.z))+IN.viewDir)));
    //half h4 = max(0,dot(useNormal,normalize(normalize(half3(IN.lightDir.x,-IN.lightDir.y,-IN.lightDir.z))+IN.viewDir)));
	...
	//------------------------------------
	half3 fresnel;
	fresnel = f0+(1.0-f0)*pow((dot(useNormal,normalize(IN.lightDir+IN.viewDir))),5);
	...
	//--------------------------------------------
	_roughness = lerp(_specsize*0.5,_specsize,irismasktex);
	float ms = pow(_roughness,4.5);
	half ndf = (ms*ms)/pow((h2*h2)*((ms*ms)-1.0)+1.0,2.0);

	//float ms2 = pow(_roughness*0.6,4.5);
	//half ndf2 = (ms2*ms2)/pow((h4*h4)*((ms2*ms2)-1.0)+1.0,2.0);

	//add caustic refraction
	//o.Albedo = saturate(lerp(o.Albedo,o.Albedo*4.0,h3*irismasktex*h2));

	//add edge specular
	o.Albedo = (o.Albedo + (fresnel * 0.7 * h));

	//add dot specular
	o.Albedo = o.Albedo + ((pow(ndf,4)) * _LightColor0);
	...
	o.Albedo = clamp(o.Albedo * _brightShift,half3(0,0,0),half3(4,4,4));
