...
class Exam15 : BaseExam
{
    ...
    public override bool CheckMove(string colliderTag, out string errorMessage)
    {
        ...
        if (CurrentTool.Instance.Tool.CodeName == "venflon" && colliderTag == "lateral_subcutaneous_arm_vein_final_target")
            _needleInsideTarget = true;

        if (CurrentTool.Instance.Tool.CodeName == "venflon" && colliderTag != "lateral_subcutaneous_arm_vein_final_target" && colliderTag != "lateral_subcutaneous_arm_vein")
        {
        ...
        if (CurrentTool.Instance.Tool.CodeName == "hand" && (colliderTag != "medial_saphenous_vein" || colliderTag != "lateral_subcutaneous_arm_vein"))
        {
            errorMessage = "Пальпируется не то место";
            return false;
        }

        this.BloodInsidePavilion(colliderTag, "lateral_subcutaneous_arm_vein_final_target");
    ...
    public override int? CheckAction(string actionCode, out string errorMessage, string locatedColliderTag = "")
    {
        ...
        if (this.VenflonInstallation(actionCode, ref errorMessage, locatedColliderTag, out returnedStep, "below_the_shoulder", "ulnar_fold", 
            "medial_saphenous_vein", "lateral_subcutaneous_arm_vein", "lateral_subcutaneous_arm_vein_final_target", ref _currentBallLiquid))
            return returnedStep;
