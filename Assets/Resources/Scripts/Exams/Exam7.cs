...
class Exam7 : BaseExam
{
    ...
    public override bool CheckMove(ref ToolItem tool, string colliderTag, out string errorMessage)
    {
        ...
        if (tool.CodeName == "syringe" && (colliderTag != "internal_jugular3_vein_final_target" || colliderTag != "internal_jugular3_vein"))
        {
            errorMessage = "Пункция не в том месте";
            return false;
        }

        if (tool.CodeName == "tweezers" && colliderTag != "disinfection_internal_jugular3_vein")
        {
            errorMessage = "Дезинфекция не в том месте";
            return false;
        }

        this.BloodInsideMove(ref tool, colliderTag, "internal_jugular3_vein_final_target");
    ...
    public override int? CheckAction(ref ToolItem tool, string actionCode, out string errorMessage, string locatedColliderTag = "")
    {
        ...
        if (this.BiosafetySpiritIodine(ref tool, actionCode, ref errorMessage, locatedColliderTag,
            "disinfection_internal_jugular3_vein", out returnedStep, true)) return returnedStep;
        ...
        //{ "anesthesia_needle",              "Взять иглу для анестезии кожи." },
        if (this.GetNeedleAction(ref tool, actionCode, ref errorMessage, "anesthesia_needle", 9)) return 10;
        ...
        if (tool.CodeName == "syringe" && actionCode == "anesthesia")
        {
            ...
            if (!locatedColliderTag.Contains("internal_jugular3_vein"))
                errorMessage = "Не то место анестезии";
            return 12;
        }

        //{ "puncture_needle",                "Взять иглу для пункции вены." },
        if (this.GetNeedleAction(ref tool, actionCode, ref errorMessage, "a45_d8_punction_needle", 12)) return 13;

        //{ "puncture_novocaine",             "Наполнить физраствором на половину." },
        if (this.HalfFillingNaCl(ref tool, actionCode, ref errorMessage)) return 14;
        ...
        if (this.CateterFinalise(ref tool, actionCode, ref errorMessage, locatedColliderTag,
            "catheter_d1", "standart_catheter_conductor", 17, out returnedStep)) return returnedStep;
