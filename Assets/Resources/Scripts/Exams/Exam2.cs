...
class Exam2 : BaseExam
{
        ...
        switch (tool.CodeName)
        {
            ...
            case "catheter":
                return new TupleList<string, string>
                {
                    ...
                    { "rotation_insertion",             "Углубить вращательными движениями" },
                    { "direct_insertion",               "Углубить прямыми движениями" }
                };
            case "patch":
                return new TupleList<string, string>
                {
    ...
    public override bool CheckMove(string colliderTag, out string errorMessage)
    {
        ...
        if (CurrentTool.Instance.Tool.CodeName == "syringe" && colliderTag != "subclavian2_vein_final_target" && colliderTag != "subclavian2_vein_target")
        {
            errorMessage = "Пункция не в том месте";
            return false;
        }

        if (CurrentTool.Instance.Tool.CodeName == "tweezers" && colliderTag != "disinfection_subclavian2_target")
        {
    ...
    public override int? CheckAction(string actionCode, out string errorMessage, string locatedColliderTag = "")
    {
        ...
        if (this.BiosafetySpiritIodine(actionCode, ref errorMessage, locatedColliderTag,
            "disinfection_subclavian2_target", out returnedStep, ref _currentBallLiquid)) return returnedStep;
        ...
        //{ "puncture_needle",                "Взять иглу для пункции вены." },
        if (this.GetNeedleAction(actionCode, ref errorMessage, "a45_d10_punction_needle", 10)) return 11;
        ...
        //{ "disconnect_syringe",             "Отсоеденяем шприц от иглы." },
        if (this.NeedleRemovingAction(actionCode, ref errorMessage, locatedColliderTag, ref _needleRemovingMoment, "subclavian_vein_final_target", 15, 20)) return 13;
